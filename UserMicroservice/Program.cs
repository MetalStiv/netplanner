...
app.MapGet("/getId", ([AllowAnonymous] async (HttpContext http,
    IUserRepositoryService userRepositoryService) => {
        var userMail = http.Request.Query["mail"];
        var user = await userRepositoryService.GetByEmailAsync(userMail);
        if (user == null)
        {
            return "";
        }

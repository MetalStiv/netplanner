...
const ProjectsTab: React.FC = observer(() => {
    ...
    const addProjectGroup = async () => {
        let addProject = await projectMicroservice.post('addProject', {
            "name": langText.userPage.projectTab.defaultGroupName,
            "groupId": projectsMetaStore.getCurrentGroupId(),
            "isGroup": true
        })
        if (addProject.status !== 200){
            ...
            <div className="startMenu">
                <div className={projectsMetaStore?.getData()
                    .filter(p => p.groupId === projectsMetaStore.getCurrentGroupId()).length === 0 ? "text" 
                        : "text text-transformed"}>{langText.userPage.projectTab.startNewProject}</div>
                <div className={projectsMetaStore?.getData()
                ...
                <div className="group-navigation-button-group">
                    <button onClick={() => projectsMetaStore
                        .setCurrentGroupId(projectsMetaStore.getById(projectsMetaStore.getCurrentGroupId())!.groupId)}>up</button>
                    <button onClick={() => projectsMetaStore.setCurrentGroupId(null)}>root</button>
                </div>
            </div>

            <div id="project-card-container">
            {
                projectsMetaStore?.getData().filter(p => p.groupId === projectsMetaStore.getCurrentGroupId()).map(p => { 
                    return p.isGroup ? 
                        <ProjectGroupCard 
                            projectId={p.id} 
                            key={`card_${p.id}`} 

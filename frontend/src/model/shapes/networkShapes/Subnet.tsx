    ...
    create() {
        return new Subnet({
            ...
            connectionPoints: [ConnectionPointTypes.CENTER]
                .map(type => {
                    const { relativeCoords, markerOffset } = calculateCPCoords(type, 45, 30);
                    return ({
                        id: '',
                        type,
                        relativeCoords,
                        markerOffset,
                        connectionAreaRadius: 26,
                        connectedShapes: null
                    });
                })
        });
...
class Subnet implements IShape {
    ...
    set overallWidth(value: number) {
        ...
        const newHeight = this.validateProperty((value * 30 / 45).toString(), GraphicalPropertyTypes.HEIGHT);
    ...
    set overallHeight(value: number) {
        const newHeigth = this.validateProperty(value.toString(), GraphicalPropertyTypes.HEIGHT);
        this.config.graphicalProperties[GraphicalPropertyTypes.HEIGHT].value = newHeigth;
        if (this.config.connectionPoints?.length) {
            this.config.connectionPoints =
                this.config.connectionPoints.map(p => ({ ...p, relativeCoords: calculateCPCoords(p.type, this.overallWidth, +newHeigth).relativeCoords }));
        }
        const newWidth = this.validateProperty((value * 45 / 30).toString(), GraphicalPropertyTypes.WIDTH);
        this.config.graphicalProperties[GraphicalPropertyTypes.WIDTH].value = newWidth;
        if (this.config.connectionPoints?.length) {
            this.config.connectionPoints =
                this.config.connectionPoints.map(p => ({ ...p, relativeCoords: calculateCPCoords(p.type, +newWidth, this.overallHeight).relativeCoords }));

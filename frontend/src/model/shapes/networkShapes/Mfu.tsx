...
interface IMfuGraphicalProps extends IShapeGraphicalProps {
    [GraphicalPropertyTypes.WIDTH]: IGraphicalProperty,
    [GraphicalPropertyTypes.FILL_COLOR_ONE]: IGraphicalProperty,
}

interface IMfuObjectProps extends IShapeObjectProps {
...
export interface IMfuConfig extends IShapeConfig {
    id?: string,
    graphicalProperties: IMfuGraphicalProps,
    objectProperties: IMfuObjectProps,
    zIndex: number,
    connectionPoints: IConnectionPoint[] | null
}

export const mfuInflater: TShapeInflater = async (messageShape: IMessageShape) => {
    if (messageShape.type !== ShapeType.MFU) {
        return null
    }
    return new Mfu({
        id: messageShape.id,
        ...
        connectionPoints: (() => {
            ...
            const h = +messageShape.graphicalProperties.find(p => p.l === GraphicalPropertyTypes.WIDTH)!.v;
...
export class MfuCreator implements IShapeCreator {
    type: ShapeType = ShapeType.MFU;
    icon: string = `<g>
            <path stroke='#1561d3' fill='#ffffff' stroke-width='2'
                d='M 6 6 l 24 0 l 0 12 l -24 0 l 0 -12 l 24 -5 m -18 17 l 0 -5 l 10 0 l 0 5' />
        </g>`
    create() {
        return new Mfu({
            graphicalProperties: {
            ...
            connectionPoints: [ConnectionPointTypes.CENTER]
                .map(type => {
                    const { relativeCoords, markerOffset } = calculateCPCoords(type, 60, 60);
                    return ({
                        id: '',
                        type,
                        relativeCoords,
                        markerOffset,
                        connectionAreaRadius: 19,
                        connectedShapes: null
                    });
                })
        });
        ...
        >
            ...
            <path
                ...
                d={`
                    ...
                    m ${+this.config.graphicalProperties[GraphicalPropertyTypes.WIDTH].value * 0.2
                    * parseInt(this.config.graphicalProperties[GraphicalPropertyTypes.MIRROR_Y]!.value.toString())}
                    ...
                    * parseInt(this.config.graphicalProperties[GraphicalPropertyTypes.MIRROR_X]!.value.toString())}
                    ...
                    m ${+this.config.graphicalProperties[GraphicalPropertyTypes.WIDTH].value * -0.2
                    * parseInt(this.config.graphicalProperties[GraphicalPropertyTypes.MIRROR_Y]!.value.toString())}
                        ${+this.config.graphicalProperties[GraphicalPropertyTypes.WIDTH].value * 0.25
                    * parseInt(this.config.graphicalProperties[GraphicalPropertyTypes.MIRROR_X]!.value.toString())}
